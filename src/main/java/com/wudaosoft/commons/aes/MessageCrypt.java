 ...
 */
package com.wudaosoft.commons.aes;
...
import com.wudaosoft.commons.utils.Base58;
...
public class MessageCrypt {
	 ...
	 */
	public static String encrypt(String text, byte[] aesKey) throws AesException {
...
//		byteCollector.addBytes(appidBytes);
		...
		try {
			...
			// 使用BASE58对加密后的字符串进行编码
			String base58Encrypted = Base58.encode(encrypted);

			return base58Encrypted;
		} catch (Exception e) {
			e.printStackTrace();
	 ...
	 */
	public static String decrypt(String text, byte[] aesKey) throws AesException {
		byte[] original;
		try {
			// 设置解密模式为AES的CBC模式
			Cipher cipher = Cipher.getInstance("AES/CBC/NoPadding");
			...
			// 使用BASE64对密文进行解码
			byte[] encrypted = Base58.decode(text);
		...
		try {
			...
			xmlContent = new String(Arrays.copyOfRange(bytes, 20, bytes.length), CHARSET);
